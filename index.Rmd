...
average_production_plot <- ggplot(average_production, aes(x = value, # reordering the country names by total renewable energy production 
  ...
  ylab("") +
  ggtitle("Average energy production by energy type from 2016-2018") 
...
ggplot(euro_table %>% filter(Year %in% c(2010,2011,2012,2013,2014,2015,2016,2017,2018)), aes(x=Year, y=Percentage, color=Country)) +
  ...
  theme(legend.position = "none",               # excluding the legend
        ...
        plot.margin = unit(c(20,20,20,20),"lines"),
        axis.title.y = element_text(margin = unit(c(0,0,0,20),"lines")),
        axis.title.x = element_text(margin = unit(c(0,0,20,0),"lines"))) + 
...
The pumped hydro power which was earlier included within the hydro power analysis, also showed it's contribution to the energy production. Although it does not produce the high amounts of energy as the other energy types, it is still being used and improved for better production. Furthermore, the pumped hydro consumes more amount of energy than it produces in 2016 & 2017, but this has improved in countries like Austria and Norway in 2018. However, they are still not producing a high amount of energy that is comparable with other renewable energy sources.
