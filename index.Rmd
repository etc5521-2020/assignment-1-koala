...
In today's day and age there has been a strong push for the reduction in production and use of fossil fuels. Per Peninsula Energy, fossil fuels "produce a significant amount of greenhouse gas emissions, including carbon dioxide methane," (Ritchie and Roser, 2018, peninsula?) These greenhouse gas emissions are known to be a main contributor to the ‘greenhouse effect’ “the warming of climate that results when the atmosphere traps heat radiating from Earth towards space,” (Twardy et al., 2007). Not only does thermal power produced by fossil fuels emit greenhouse gases, it is not renewable, meaning it cannot be “replaced naturally in a short period of time”. This causes energy producers and companies to charge consumers based on demand, and as demand fluctuates, this results in long-term price volatility.

Per Our World Data, Europe is currently the 3rd highest consumer of primary energy behind Asia Pacific and North America making up approximately 15% of the total World Energy Consumption (Ritchie and Roser, 2018).  

As a result, the European power sector currently plays a crucial role in attaining targets to “cut green-house gases by 40% by 2030, compared to 1990” (Jones et al., 2017) in favor of cleaner and more renewable energy sources. For the purposes of this analysis we will be using Eurostat European Energy data (Eurostat, 2020) to assess how successful they currently are and answer the questions:  
...
The data in the tidytuesday package only contains yearly totals from 2016-2018, (Ellis, 2020). However, monthly data can be found on the Eurostat website including data from 2010 until May 2020 as at 14-08-20. The statistics are collected and compiled by Member States. Eurostat prepared a template to gather this information, (Eurostat, n.d.).
...
Looking at the metadata and dataset, we notice that the pumped hydro power and hydro power have been put together as electricity produced by hydro power. When researching into hydro power and pumped hydro power, it can be said that they are different ways of producing electricity using water as their source (Petrescu, 2015).

Hence, we can find the amount of electricity produced by each of the techniques using a bar graph such as \@ref(fig:cmp). We notice that the electricity produced by hydro power is higher than that of pumped hydro power. For example, Norway clearly shows that there is a huge amount of difference in the electricity production where the energy produced by hydro power is **`r  hydro_pro %>% filter(country_name == "Norway" & year == 2016) %>% pull(avg_energy)` ** Gwh for 2016, **`r  hydro_pro %>% filter(country_name == "Norway" & year == 2017) %>% pull(avg_energy)` ** Gwh for 2017, **`r  hydro_pro %>% filter(country_name == "Norway" & year == 2018) %>% pull(avg_energy)`** Gwh for 2018. Whereas, electricity produced by the same country through the pumped hydro power technique is **`r  electricity_prod %>% select(country_name,pumped_hydro_power) %>%  filter(country_name == "Norway" & year == 2016) %>% pull(pumped_hydro_power)`** Gwh for 2016, **`r  electricity_prod %>% select(country_name,pumped_hydro_power) %>%  filter(country_name == "Norway" & year == 2017) %>% pull(pumped_hydro_power)` ** Gwh for 2017, **`r  electricity_prod %>% select(country_name,pumped_hydro_power) %>%  filter(country_name == "Norway" & year == 2018) %>% pull(pumped_hydro_power)`** Gwh for 2018.
...
target <- 
ggplot(euro_table %>% filter(Year %in% c(2010,2011,2012,2013,2014,2015,2016,2017,2018)), aes(x=Year, y=Percentage, color=Country)) + #filtering only the years 2010-2018
  geom_line() +
  ...
  xlab("") + #adding x-axis label
  ylab("Renewable energy (%)") + #adding y-axis label
  ggtitle("Renewable energy % share of total energy consumed for EU countries from 2010 - 2018") + #adding a plot title
...
Albania and Montenegro have been going down in the share of their renewable energy. Montenegro has reported a 5.3% reduction dropping from *`r euro_table %>% filter(Country == "Montenegro" & Year == 2014) %>% select(Percentage)`%* in 2014 to *`r euro_table %>% filter(Country == "Montenegro" & Year == 2018) %>% select(Percentage)`%* in 2018. Albania has seen an inconsistent trend and recently dropped to *`r euro_table %>% filter(Country == "Albania" & Year == 2018) %>% select(Percentage)`%* in 2018 from an all time high of *`r euro_table %>% filter(Country == "Albania" & Year == 2016) %>% select(Percentage)`%* in 2016.
...
Ellis Hughes (2020). tidytuesdayR: Access the Weekly 'TidyTuesday' Project Dataset.
https://CRAN.R-project.org/package=tidytuesdayR

European Commission. (n.d.). *Energy Innovation made in the EU*. Retrieved September 17, 2020, from https://media.giphy.com/media/3oxHQlREH7ufJK4yYg/giphy.gif

Eurostat. (2018). *Share of Renewable Energy in Gross Final Energy Consumption*. 
Retrieved September 13, 2020, from https://ec.europa.eu/eurostat/tgm/table.do?tab=table&plugin=1&language=en&pcode=t2020_31

Eurostat. (n.d.). *Your key to European Statistics*. Retrieved August 26, 2020, from https://ec.europa.eu/eurostat/web/energy/methodology
...
Jones, D., Sakhel, A., Buck, M., Graichen, P., (2018, January). *The European Power Sector in 2017*. Retrieved August 26, 2020, from https://ember-climate.org/wp-content/uploads/2018/01/EU-power-sector-report-2017.pdf

Mock, J., Scherer, C. (2020). *European Energy*. Retrieved September 13, 2020, from https://github.com/rfordatascience/tidytuesday/blob/master/data/2020/2020-08-04/readme.md

Nicholas Tierney, Di Cook, Miles McBain and Colin Fay (2020). naniar: Data
Structures, Summaries, and Visualisations for Missing Data. R package version
0.5.2. https://CRAN.R-project.org/package=naniar

Peninsula Clean Energy. (2018). *What is the difference between clean and renewable energy?*. Retrieved August 26, 2020, from https://www.peninsulacleanenergy.com/faq-items/what-is-the-difference-between-clean-and-renewable-energy/

Petrescu, F., Petrescu, R. (2015). *Hydropower and Pumped-storage*. Retrieved September 16, 2020, from 
https://www.altenergymag.com/article/2015/11/hydropower-and-pumped-storage/22104#:~:text=The%20pumped%20storage%20provides%20a,enabling%20additional%20system%20peak%20capacity.&text=Hydropower%20or%20water%20power%20is,be%20harnessed%20for%20useful%20purposes.

R Core Team (2020). R: A language and environment for statistical computing. R Foundation for Statistical Computing, Vienna, Austria. https://www.R-project.org/

Ritchie, H., Roser, M. (2018) *Energy*. Retrieved August 26, 2020, from https://ourworldindata.org/energy

Tribout-Maurizi, A. (2019). *FRANCE*. Retrieved August 27, 2020, from https://cnpp.iaea.org/countryprofiles/France/France.htm

Twardy, S., Starr, C., DeWitt, S., Frostic, M., (2007). *Greenhouse Gases Effect on Global Warming*. Retrieved August 26, 2020, from https://svs.gsfc.nasa.gov/20114
