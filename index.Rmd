...
p <- contribute_to_europe %>%
    pivot_longer(cols=c(2:8),
                 ...
                 values_to = "value")
 p %>%
   plot_ly(labels = ~contribution,
          values = ~value,
          textposition = 'outside',
          textinfo = 'label+percent') %>%
   add_pie(data = filter(p, year==2016), name = "2016", domain = list(row = 0, column = 0)) %>%
  add_pie(data = filter(p, year==2017), name = "2017", domain = list(row = 0, column = 2)) %>%
  add_pie(data = filter(p, year==2018), name = "2018",domain = list(row = 0, column = 4)) %>%
   layout(title = 'Energy source contribution to the total production in Europe (%) 2016,2017,2018',
...
df_tbl %>%
  ggplot(aes(x=country_name, y= value, fill= contribution)) +
