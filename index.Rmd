...
p2 <- ggplot(electricity_prod,aes(x = reorder(country_name,-`Pumped hydro power`), y = `Pumped hydro power`, fill = year)) + 
  geom_col() +
  theme(axis.text.x = element_text(angle = 90),legend.position = "none") +
  xlab("country name") + 
  facet_wrap(~ year) +
...
subplot(diff_plotly,hydro_plotly,margin = 0.23,titleY = TRUE, titleX = TRUE) %>% 
  layout(title = "Hydro power                        vs                    Pumped Hydro power") 
...
b <- list(
  title = "Difference in electricity produced and absorbed"
)
...
prod_plot_16 <- ggplot(binded_hydro %>% filter(year == 2016),
  ...
  # ggtitle("Electricity produced by Pumped Hydro vs Spent on Pumped Hydro")+
  coord_flip()
...
#gridExtra::grid.arrange(p3,p4,p5)
subplot(p3,p4,p5,margin = 0.05,titleY = TRUE,titleX = TRUE) %>% layout(title = "Difference between electricity produced and absorbed by pumped hydro power") 
```
