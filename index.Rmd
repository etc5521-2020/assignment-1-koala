...
p2 <- p2 <- ggplot(electricity_prod,aes(x = reorder(country_name,-pumped_hydro_power), y = pumped_hydro_power, fill = year)) + 
  geom_col() +
  theme(axis.text.x = element_text(angle = 90),legend.position = "none") +
  xlab("country name") + 
  facet_wrap(~ year) +
...
subplot(diff_plotly,hydro_plotly,margin = 0.10,titleX = TRUE,titleY = TRUE) %>% 
  ...
  layout(title = "Diff in electricity produced by pumped hydro & hydro power", 
...
Looking at the metadata and dataset, we notice that the pumped hydro power and hydro power have been put together as electricity produced by hydro power, when checked whether hydro power and pumped hydro power, it can be said that they are different ways of producing electricity using water as their source (@altenergymag) .

Hence, we can find the amount of electricity produced by each of the techniques using a bar graph such as \@ref(fig:cmp), we notice that the electricity produced by hydro power is higher than the that of pumped hydro power. for example, when we consider the country Norway for all the years (2016,2017,2018) clearly it shows that there is huge amount of difference in the electricity production where it is **`r  hydro_pro %>% filter(country_name == "Norway" & year == 2016) %>% select(country_name,avg_energy)` ** for year 2016,**`r  hydro_pro %>% filter(country_name == "Norway" & year == 2017) %>% select(country_name,avg_energy)` ** for year 2017, **`r  hydro_pro %>% filter(country_name == "Norway" & year == 2018) %>% select(country_name,avg_energy)`** for year 2018 , whereas, electricity produced by the same country by using the pumped hydro power technique is **`r  electricity_prod %>% select(country_name,pumped_hydro_power) %>%  filter(country_name == "Norway" & year == 2016) %>% select(pumped_hydro_power)`** for year 2016 , **`r  electricity_prod %>% select(country_name,pumped_hydro_power) %>%  filter(country_name == "Norway" & year == 2017) %>% select(pumped_hydro_power)` ** for year 2017, **`r  electricity_prod %>% select(country_name,pumped_hydro_power) %>%  filter(country_name == "Norway" & year == 2018) %>% select(pumped_hydro_power)`** for the year 2018.

Just like Norway, every other country has a very low amount of pumped hydro power produced from that of the hydro power within the country. Some of the countries like Georgia, Serbia have very low counts of hydro power but even less or ~ 0 amount of electricity produced by either of the two techniques.

## How efficient is it produce electricity using Pumped Hydro power?

```{r pumped-power,message=FALSE,warning=FALSE,fig.cap= "Difference between electricity produced & absorbed by pumped hydro power"}
hydro <- energy_types %>% 
...
subplot(p3,p4,p5,nrows =1,margin = 0.11,titleY = TRUE,titleX = TRUE ) %>% 
  #adding titles and the axis labels/titles for the subplot object
  layout(title = "Diff btwn electricity produced & absorbed by pumped hydro power",
yaxis = list(title = "Country"),
yaxis2 = list(title = ""),
yaxis3 = list(title = ""),
xaxis = list(title = ""),
...
The figure \@ref(fig:pumped-power) shows a bar graph with countries on its y-axis the difference in energy produced & absorbed within each of those countries across three years. **The years 2016, 2017 have a negative values which indicates that the energy used by the pumped hydro power technique was higher than the energy it produced **, although 2018 is also similar and holds true to the above statement, there are some countries which have improved its consumption:production ratio over the years. For instance, let's have a look at  **`r binded_hydro %>% filter(country_name == "Austria" & year == 2016) %>% select(energy_used)`** but in **`r binded_hydro %>% filter(country_name == "Austria" & year == 2018) %>% select(energy)`** and was one of the top countries for having a huge turn over in the consumption:production. Countries like *`r binded_hydro %>% filter(country_name == "Germany" & year == 2016) %>% select(energy_used)`* , *`r binded_hydro %>% filter(country_name == "Germany" & year == 2017) %>% select(energy_used)`* and in *`r binded_hydro %>% filter(country_name == "Germany" & year == 2018) %>% select(energy_used)`* have reduced the amount energy consumed/absorbed for production of energy.
...
ggplot(euro_table %>% filter(Year %in% c(2010,2011,2012,2013,2014,2015,2016,2017,2018)), aes(x=Year, y=Percentage, color=Country)) +
  ...
  facet_wrap(~Country) #display a line graph for each country
...
From the graph, countries like Iceland, Finland, Norway and Sweden have the highest share of renewable energy. Norway consumed 72.75% of it's energy as renewable energy in 2018, followed by Iceland at 72.18%, Norway at 54.64% and Finland at 41.16%. Although Iceland and Norway don't have a 2020 target, they are well above their European counterparts and it is fair to say they have surpassed all renewable energy target level. Finland has surpassed their 2020 target level in 2014 and Sweden all the back in 2012!

Cyprus, Denmark, Germany and the United Kingdom have a trend that is most noticeable to see as countries with the share of renewable energy consistently increasing over the years. Cyprus has increased from 6.1% in 13.8% and have already surpassed their 2020 target level of 13%! Denmark reported the largest increase over the 9 year period from 21.8% to 35.7% and reached their 2020 target of 30% way back in 2015! Germany has increased 11.6% to 16.4% and are on track to reach their 2020 target of 18%. The United Kingdom has increased from 3.7% to 11% and are on track to reach their 2020 target of 15%.

Albania and Montenegro have been going down in the share of their renewable energy. Montenegro has reported a 5.2% reduction dropping from 44% in 2014 to 38.8% in 2018. Albania has seen an inconsistent trend and recently dropped to 34.8% in 2018 from an all time high of 35.5% in 2016.
...
The pumped hydro power which was earlier included within the hydro power analysis, also showed it's contribution to the energy production, although it does not produce the high amounts of energy as the other energy types, it is still being used and improved for better production. Furthermore, the pumped hydro consumes more amount of energy than it produces, although this has been improved in places like Austria,Norway, etc. over the years, they are still not producing a high amount of energy that is needed. 
