...
prod <- energy_types %>% distinct(type) #select(-level)%>%  filter(year == 2017)
  #filtering the energy types to be hydro and pumped hydro for comparison
  dplyr::filter(type ==c("Hydro" ,"Pumped hydro power"))
...
subplot(diff_plotly,hydro_plotly,margin = 0.23) %>% 
  layout(title = "Pumped hydro power                        vs                      Hydro power") 
...
#plotting the above binded datframe on a bar graph 
prod_plot <- ggplot(binded_hydro,
  ...
  xlab("Country Name") +
  ylab("Difference in production/consumption") +
