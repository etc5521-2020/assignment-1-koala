...
energy_types %>%
  ...
  geom_col() +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(title = "Average Renewable vs Non-Renewable Energy Production by Country from 2016-2018",
         x = "")
```
...
country_totals %>%
  ...
  summarise(Exports = sum(Exports),
            Imports = sum(Imports),
            `Total net production` = sum(`Total net production`))
```
```{r}
country_totals %>%
  group_by(country_name,type) %>%
  ...
  summarise(Exports = sum(Exports),
            Imports = sum(Imports),
            `Total net production` = sum(`Total net production`)) %>%
  mutate(`Net Imports and Exports` = Exports + Imports )
```
